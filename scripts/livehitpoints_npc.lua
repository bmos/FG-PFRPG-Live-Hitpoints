...
local function reportHdErrors(nodeNPC, sHd)
    ...
    if (sNpcName ~= '') and sHdErrorEnd and DataCommon.isPFRPG() and
        not bErrorAlerted then
        ChatManager.SystemMessage(string.format(
                                      Interface.getString('npc_hd_error_pf1e'),
                                      sNpcName))
        if (sError ~= '') then
            ChatManager.SystemMessage(string.format(
                                          Interface.getString(
                                              'npc_hd_error_type'), sError,
                                          sError))
        end
        DB.setValue(nodeNPC, 'erroralerted', 'number', 1)
    elseif (sNpcName ~= '') and sHdErrorEnd and not bErrorAlerted then
        ChatManager.SystemMessage(string.format(
                                      Interface.getString('npc_hd_error_generic'),
                                      sNpcName))
        if (sError ~= '') then
            ChatManager.SystemMessage(string.format(
                                          Interface.getString(
...
local function upgradeNpc(nodeNPC, rActor, nLevel, nCalculatedAbilHp, nHdAbilHp,
    ...
    local nMiscMod = nHdAbilHp - nCalculatedAbilHp -
                         getFeatBonusHp(nodeNPC, rActor, nLevel)
...
local function getAbilityBonusUsed(nodeNPC, rActor, nLevel, nAbilHp, bOnAdd)
    ...
    if sAbility == '' then
        if string.find(string.lower(DB.getValue(nodeNPC, 'type', '')), 'undead',
                       1) and DataCommon.isPFRPG() then
            sAbility = 'charisma'
    ...
    if bOnAdd or not DB.getValue(nodeNPC, 'livehp.total') or
        nodeNPC.getParent().getNodeName() == 'npc' then
        upgradeNpc(nodeNPC, rActor, nLevel, (nAbilityMod * nLevel) or 0,
                   nAbilHp, bOnAdd)
    end
...
function setHpTotal(rActor, bOnAdd)
    ...
    local nTotalHp = LiveHP.calculateHp(nodeNPC, rActor, getAbilityBonusUsed(
                                            nodeNPC, rActor, nLevel or 0,
                                            nHdAbilHp, bOnAdd), getFeatBonusHp(
                                            nodeNPC, rActor, nLevel or 0))
...
local function onEffectChanged(node)
    ...
    if not ActorManager.isPC(rActor) and
