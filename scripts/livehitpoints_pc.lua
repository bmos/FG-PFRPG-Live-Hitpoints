...
local function hasSpecialAbility(nodeActor, sSearchString, bFeat, bTrait,
    ...
    if bFeat then
        for _, vNode in pairs(DB.getChildren(nodeActor, '.featlist')) do
            local vLowerSpecAbilName = StringManager.trim(DB.getValue(vNode,
                                                                      'name', ''):lower());
            local nRank = tonumber(vLowerSpecAbilName:match(sLowerSpecAbil ..
                                                                ' (%d+)', 1));
            if vLowerSpecAbilName and
                (nRank or vLowerSpecAbilName:match(sLowerSpecAbil, 1)) then
...
local function getAbilityBonusUsed(nodePC, rActor, nLevel)
    ...
    local nAbilityMod = DB.getValue(nodePC,
                                    'abilities.' .. sAbility .. '.bonus', 0)
    local nEffectBonus = math.floor((EffectManager35EDS.getEffectsBonus(rActor,
                                                                        {
        DataCommon.ability_ltos[sAbility]
    }, true) or 0) / 2)
...
function setHpTotal(rActor)
    ...
    local nTotalHp = LiveHP.calculateHp(nodePC, rActor, getAbilityBonusUsed(
                                            nodePC, rActor, nLevel),
                                        getFeatBonusHp(nodePC, nLevel))
...
local function onEffectChanged(node)
    ...
    if ActorManager.isPC(rActor) and
        LiveHP.checkEffectRelevance(node.getChild('..')) then
        setHpTotal(rActor)
    end
end
...
function applyClassStats_new(nodeChar, nodeClass, nodeSource, nLevel,
                             nTotalLevel, ...)
    ...
    if sHDSides then
        ...
        elseif OptionsManager.getOption('LURHP') == 'on' then
            ...
            local sFormat =
                Interface.getString("char_message_classhppromptroll");
            local sMsg = string.format(sFormat, "d" .. nHDSides,
...
function onFavoredClassBonusSelect_new(aSelection, rFavoredClassBonusSelect, ...)
    ...
    if aSelection[1] == Interface.getString("char_value_favoredclasshpbonus") then
        ...
        DB.setValue(nodeChar, "livehp.misc", "number",
                    DB.getValue(nodeChar, "livehp.misc", 0) + 1)
...
function onInit()
    if Session.IsHost then
        DB.addHandler(DB.getPath('charsheet.*.abilities.*.bonus'), 'onUpdate',
                      onAbilityChanged)
        DB.addHandler(DB.getPath('charsheet.*.abilities.*.bonusmodifier'),
                      'onUpdate', onAbilityChanged)
        DB.addHandler(DB.getPath('charsheet.*.abilities.*.damage'), 'onUpdate',
                      onAbilityChanged)

        DB.addHandler(DB.getPath('charsheet.*.featlist.*.name'), 'onUpdate',
                      onFeatsChanged)
