...
local function getFeatBonus35E(rActor)
	...
	local nLevel = DB.getValue(nodePC, "level", 0)
	local nFeatBonus = 0
	if CharManager.hasFeat(nodePC, "Toughness", true) then
		nFeatBonus = nFeatBonus + 3
	end
	if CharManager.hasFeat(nodePC, "Improved Toughness", true) then
...
function setHpTotal(rActor)
	...
	DB.setValue(
		...
		"hp.total",
		"number",
		LiveHP.calculateHp(nodePC, rActor, getAbilityBonusUsed(rActor), getFeatBonusHp(rActor))
	)
end
...
local function applyClassStats_new(nodeChar, nodeClass, nodeSource, nLevel, nTotalLevel, ...)
	...
	local function getHD()
		...
		if DataCommon.classdata[sClassLookup:lower()] and not sHD:match("^%d?d%d+") then
			sHD = DataCommon.classdata[sClassLookup:lower()].hd
		end
		return sHD
	end
...
local function onFavoredClassBonusSelect_new(aSelection, rFavoredClassBonusSelect, ...)
	...
	for k, v in ipairs(aSelection) do
		if v == Interface.getString("char_value_favoredclasshpbonus") then
			...
			setHpTotal(ActorManager.resolveActor(rFavoredClassBonusSelect.nodeChar))
...
local function onEffectChanged(node)
	...
	if ActorManager.isPC(rActor) and LiveHP.checkEffectRelevance(DB.getChild(node, "..")) then
		setHpTotal(rActor)
	end
end
